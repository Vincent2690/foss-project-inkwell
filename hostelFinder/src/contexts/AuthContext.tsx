...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [userRole, setUserRole] = useState<'student' | 'landlord' | 'admin' | null>(null);
  ...
  const fetchUserRole = async (userId: string) => {
    try {
      ...
      setUserRole(data?.role as 'student' | 'landlord' | 'admin' || null);
    } catch (error) {
      console.error('Error fetching user role:', error);
    }
  };

  const signUp = async (email: string, password: string, fullName: string, role: 'student' | 'landlord') => {
    try {
